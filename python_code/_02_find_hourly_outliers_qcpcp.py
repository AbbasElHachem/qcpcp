...
def plot_config_event_radar(event_date,
                            stn_one_id, stn_one_xcoords,
                            stn_one_ycoords,
                            ppt_stn_one, interp_ppt,
                            stns_ngbrs, ppt_ngbrs,
    ...
    # read radar file
    # read radolan file
    rwdata, rwattrs = wrl.io.read_radolan_composite(radar_file)
...
#     utm32 = "+init=EPSG:32632"
    ...
    minx = np.nanmin(x_ngbrs) - 2000
    miny = np.nanmin(y_ngbrs) - 2000

    maxx = np.nanmax(x_ngbrs) + 2000
    maxy = np.nanmax(y_ngbrs) + 2000

    # colormap
    bound_ppt = np.arange(0.0, 12.1, 2)
    bound_ppt[0] = 0.1
    interval_ppt = np.linspace(0.1, 0.95)
    colors_ppt = plt.get_cmap('YlGnBu')(interval_ppt)
    cmap_ppt = LinearSegmentedColormap.from_list(
        'name', colors_ppt)
...
#                     val_float_format = '% 0.1f mm' % (time_idx_val[0])
    ...
    cax2 = divider0.append_axes("right", size="2%", pad=0.3)

    cb1 = fig.colorbar(pm, ax=ax, cax=cax2, extend='min',
                       label='Radar precipitation estimates %s'
                       % r'$[mm.hr^{-1}]$')
...
if __name__ == '__main__':
    ...
    path_to_radar_image_1_bf = (
        r"X:\staff\elhachem\GitHub\qcpcp"
        r"\example_data\raa01-rw_10000-0805140750-dwd---bin"
    )

    path_to_radar_image_1_af = (
        r"X:\staff\elhachem\GitHub\qcpcp"
        r"\example_data\raa01-rw_10000-0805140850-dwd---bin"
    )

    path_to_radar_image_2_af = (
        r"X:\staff\elhachem\GitHub\qcpcp"
        r"\example_data\raa01-rw_10000-0805311750-dwd---bin"
    )

    path_to_radar_image_2_bf = (
        r"X:\staff\elhachem\GitHub\qcpcp"
        r"\example_data\raa01-rw_10000-0805311850-dwd---bin"
    )
    ...
    # EXAMPLE FROM PAPER - uncomment if not needed
    target_stn = 'P03231'

    target_events_dict = {
        '2008-05-14 08:00:00': [path_to_radar_image_1_bf,
                                path_to_radar_image_1_af],
        '2008-05-31 18:00:00': [path_to_radar_image_2_bf,
                                path_to_radar_image_2_af]}
    ...
    for ex, event_date in enumerate(stn_df_biggest_interp.index):
        try:
            # example event from paper - uncomment if not needed
            if str(event_date) in target_events_dict.keys():
                ...
                if diff_obsv_interp_O0 >= 3:
                    ...
                    ax.set_ylabel('Precipitation %s' % r' $[mm.h^{-1}]$')
                    ...
                    if str(event_date) in target_events_dict.keys():
                        ...
                        plot_config_event_radar(event_date=event_date,
                                                stn_one_id=stn_id, stn_one_xcoords=x_dwd_interpolate,
                                                stn_one_ycoords=y_dwd_interpolate,
                                                ppt_stn_one=obsv_ppt_0,
                                                interp_ppt=interpolated_vals_O0_orig,
                                                ...
                                                y_ngbrs=y_dwd_all,
                                                out_dir=out_dir,
                                                radar_file=target_events_dict[str(
                                                    event_date)][0],
    ...
    stn_df_biggest_interp.to_csv(
        os.path.join(out_dir,
                     'interp_obsv_vals_%s.csv' % stn_id),
        sep=';')
